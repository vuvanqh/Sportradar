...
import type { CreateTeamEvent } from "../../types/createEventTypes";
import { useTeams } from "../../customHooks/useTeamDetails";
type eventModalProps = {
    open: boolean,
    onClose: ()=>void
}

export default function CreateTeamEventModal({open = false, onClose}: eventModalProps){
    ...
    async function handleAction(_: any, formData: FormData) {
        ...
        if (!homeTeam || !awayTeam) {
            return { error: "Both teams must be selected" };
        }
        ...
        const createEventReq: CreateTeamEvent = {
    ...
    return <Modal open={open} onClose={onClose}>
       <form className="event-form" action={formAction}>
            ...
            <Select placeholder={sport ? "Select home team..." : "Select sport first"} isDisabled={!sport} options={filteredTeams.filter(t=>t.teamId!=awayTeam).map(c => ({ value: c.teamId, label: c.teamName }))} onChange={(val) => setHomeTeam(val?.value ?? "")}/>
            <Select placeholder={sport ? "Select away team..." : "Select sport first"} isDisabled={!sport} options={filteredTeams.filter(t=>t.teamId!=homeTeam).map(c => ({ value: c.teamId, label: c.teamName }))} onChange={(val) => setAwayTeam(val?.value ?? "")}/>
            <input type="hidden" name="homeTeamId" value={homeTeam} />
            <input type="hidden" name="awayTeamId" value={awayTeam} />
